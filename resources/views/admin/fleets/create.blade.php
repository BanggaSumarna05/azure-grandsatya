...
@section('content')
    <div class="max-w-3xl mx-auto px-6 lg:px-12">
        <h1 class="font-display text-4xl tracking-tightest mb-12">Tambah Armada</h1>

        <form action="{{ route('admin.fleets.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            @csrf
            @if ($errors->any())
            ...
            <div class="space-y-3">
                ...
                <input type="text" id="name" name="name" required class="w-full bg-white border border-gs-stone px-5 py-4 focus:ring-0 focus:border-gs-ink transition-colors duration-300">
            </div>

            <div class="space-y-3">
                <label for="class" class="block text-xs tracking-wider uppercase text-gs-slate">Kelas</label>
                <input type="text" id="class" name="class" required class="w-full bg-white border border-gs-stone px-5 py-4 focus:ring-0 focus:border-gs-ink transition-colors duration-300">
            </div>

            <div class="space-y-3">
                <label for="capacity" class="block text-xs tracking-wider uppercase text-gs-slate">Kapasitas</label>
                <input type="number" id="capacity" name="capacity" required class="w-full bg-white border border-gs-stone px-5 py-4 focus:ring-0 focus:border-gs-ink transition-colors duration-300">
            </div>
            ...
            <div class="space-y-3">
                ...
                <textarea id="description" name="description" rows="4" class="w-full bg-white border border-gs-stone px-5 py-4 focus:ring-0 focus:border-gs-ink transition-colors duration-300 resize-none"></textarea>
            </div>
